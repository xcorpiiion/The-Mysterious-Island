...
public class Atirar : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //UI
        BalasExtra.text = armas[armaAtual].balasExtra.ToString();
        BalasPente.text = armas[armaAtual].balasNoPente.ToString();
        //troca de armas
        ...
        if (Input.GetMouseButtonDown(0) && armas[armaAtual].balasNoPente > 0 && recarregando == false && atirando == false)
        {
            ...
            {
                if (pontoDeColisao.transform.gameObject.tag == "Inimigo")
                {
     
                    pontoDeColisao.transform.gameObject.GetComponent<Inimigo>().vida -= armas[armaAtual].danoPorTiro;
                    pontoDeColisao.transform.gameObject.GetComponent<Inimigo>().animacao.SetBool("damage", true);
                }
            }
        } else {

            
                //inimigo.animacao.SetBool("damage", false);
